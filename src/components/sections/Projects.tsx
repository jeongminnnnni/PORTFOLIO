...
function ProjectItem({ project, order }: { project: Project; order: number }) {
  return (
    <article
      id={project.id}
      className="scroll-mt-24 border-b border-line py-12 first:pt-0 last:border-b-0 last:pb-0"
    >
      <Reveal>
      ...
      <Reveal delay={0.05}>
        <dl className="mt-8 grid grid-cols-1 gap-x-8 gap-y-6 md:grid-cols-[7rem_1fr]">
          ...
          {project.note ? (
            <>
              <dt className="text-label pt-1 text-ink-muted">표기 주석</dt>
              <dd className="border-l-2 border-accent pl-4 text-sm leading-relaxed text-ink">
